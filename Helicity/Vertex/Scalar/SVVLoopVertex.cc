...
void SVVLoopVertex::setCoupling(Energy2, tcPDPtr, tcPDPtr,
  ...
  for(unsigned int i=0;i<masses.size();++i) {
    double mls = sqr(masses[i] * UnitRemoval::InvE);
    if(type[i] == PDT::Spin1Half) {
      ...
      a += 2.*(left[i]+right[i])*(masses[i]*UnitRemoval::InvE)*(-2.*B0(ps2,mls,mls)+ 
					    C0*(pv1s + pv2s - ps2)
					    + 8.*C00);
      b += 8.*(left[i]+right[i])*(masses[i]*UnitRemoval::InvE)*(C0 + 3.*C1 +3.*C2 +
					    2.*(C11 + 2.*C12 + C22));
      c += 4.*(left[i]+right[i])*(masses[i]*UnitRemoval::InvE)*(C0 +2.*(2.*C1+C2 + 
						    2.*(C11 +C12)));
      d += 4.*(left[i]+right[i])*(masses[i]*UnitRemoval::InvE)*(C0 + 4.*(C1+C11+C12));
      e += 8.*(left[i]+right[i])*(masses[i]*UnitRemoval::InvE)*(C1 + 2.*C11);
      f += 4.*(left[i]-right[i])*(masses[i]*UnitRemoval::InvE)*C0;
    }
	...
	B1C(B1(pv2s,mls,mls)),B11C(B11(pv2s,mls,mls));
      ...
      // coefficient
      a += 0.5*left[i]*(B0A*(2.*mls2*(-6.*mls + pv1s + pv2s) 
			     + mls*(-2.*mls + pv1s + pv2s)*ps2
			     ) 
			+ 2.*(8.*mls3*C0A*pv1s 
			      - 2.*mls2*C0A*(pv1s*pv1s) 
			      + 2.*mls*B00B*pv2s 
			      + 8.*mls3*C0A*pv2s 
			      + mls*B0B*pv12 
			      + mls*B0C*pv12

...
void MPISampler::initialize() {
  ...
  theSampler.maxTry(theMPIHandler->maxLoop());

  bool nozero = false;
  for ( int i = 0, N = theMPIHandler->nBins(); i < N; ++i )
    if ( theSampler.addFunction(theMPIHandler->nDim(i), theMPIHandler) )
      nozero = true;
  if ( !nozero ) throw EventInitNoXSec()
    << "The UnderlyingEvent handler '" << theMPIHandler->name()
...
double MPISampler::generate() {
  if ( !theSampler.generate() ) throw EventLoopException()
    << "The maximum number of attempts per event (" << theMPIHandler->maxLoop()
    << ") in multiple interaction handler '" << theMPIHandler->name() << "' was exceeded."
...
void MPISampler::dofinish() {
  if ( theSampler.n() <= 0 &&
       theMPIHandler && theMPIHandler->statLevel() > 1 ) {
    generator()->log()
      << "No events generated by the MPIsampler '" << name() << "'" << endl;
  }
  else if ( theMPIHandler && theMPIHandler->statLevel() > 1 )
    generator()->log()
...
void MPISampler::persistentOutput(PersistentOStream & os) const {
  os << theEps << theMargin << theNTry << theMPIHandler;
  theSampler.output(os);
}

void MPISampler::persistentInput(PersistentIStream & is, int) {
  is >> theEps >> theMargin >> theNTry >> theMPIHandler;
  theSampler.input(is);

...
void EvtGen::doinitrun() {
  ...
  generator()->log() << "Initializing EvtGen " << endl;
  generator()->log() << "Storing known decay models " << endl;
  ...
  cout << "testing conversion from ThePEG to EvtGen " << endl;
  EvtId etemp;
  for(;pit!=pend;++pit)
    {
      cout << pit->first << "     ";
      etemp=EvtGenID(pit->first);
      if(etemp.getAlias()>=0)
	{
      ...
      else{cout << pit->second->PDGName()  << " has no match in EvtGen " << endl;}
    }
  ...
  for(int ix=0;ix<EvtPDL::entries();++ix)
    {
      cout << EvtPDL::partlist()[ix].getStdHep() << "     ";
      cout << EvtPDL::partlist()[ix].getName();
      idtemp=ThePEGID(EvtPDL::partlist()[ix].getId());
      ...
      if(ptemp)
	{
      ...
      else{cout << " has no match in ThePEG " << endl;}
    }
  exit(1);
  */
  /*
  ...
  int pegid[7];
  ofstream output("evtgendecay.output");
  for(int ix=0;ix<EvtDecayTable::getNMode(parent.getAlias());++ix)
    {
      cout << "testing in loop " << ix << " " 
	   << EvtDecayTable::getNMode(parent.getAlias()) << endl;
      EvtDecayBase* decayer=EvtDecayTable::getDecay(parent.getAlias(),ix);
      if(decayer->getNDaug()>7)
	{cout << "too many decay products " << endl;}
      else
	{
	  for(int iy=0;iy<7;++iy)
	    {
	      if(iy<decayer->getNDaug())
		{
		  try
		    {
		      pegid[iy]=ThePEGID(decayer->getDaug(iy));
		    }
		  catch(Exception)
      ...
      output << "insert into decay_modes (incomingID,BR,decayon,star,outgoingID1,"
	     << "outgoingID2,outgoingID3,outgoingID4,outgoingID5,outgoingID6,outgoingID7,"
	     << "description,decayer) values (" << parentid << "," << br << ",'on','*',";
      for(int iy=0;iy<7;++iy)
	{output << pegid[iy] << ",";}
      output << "'Decay of %name% with branching ratio taken from EvtGen.',3);" << endl;
...
void EvtGen::persistentOutput(PersistentOStream & os) const {
  os << _decayname << _pdtname << _maxtry << _maxunwgt;
}

// persistent input
void EvtGen::persistentInput(PersistentIStream & is, int) { 
  is >> _decayname >> _pdtname >> _maxtry >> _maxunwgt;
}
...
void EvtGen::ThePEGSpin(PPtr peg,EvtParticle *evt)
{
  ...
  if(evt->getSpinType()==EvtSpinType::STRING||
     ...
     evt->getSpinType()==EvtSpinType::SPIN7HALF)
    {return;}
  ...
  for(ix=0;ix<nstate;++ix)
    {
      ...
      if(inspin!=PDT::Spin0){eind[iloc]=hind[0];++iloc;}
...
EvtId EvtGen::EvtGenID(int id)
{
  EvtId output;
  int absid=abs(id),ispin(absid%10),isgn(id/absid);
  // handle the easy cases
  if(absid<=8|| //quarks(+excited)
     (absid>=11&&absid<=18)|| // leptons(+exicted)
     (absid>=21&&absid<=25)||(absid>=32&&absid<=37)|| // SM(+extensions) gauge bosons and higgs
     (absid>100&&absid<600&&ispin%2==1&&ispin<=9)|| // 1 1S0, 1 3S1 and 1 3P2 1 3D3 mesons are the same
     (absid>10100&&absid<10600&&(ispin==3))|| // 1 1P1 mesons are the same
     (absid>20100&&absid<20600&&(ispin==3))|| // 1 3P1 mesons are the same
     (absid>=100&&absid<=3000&&ispin==0) // mixed kaons and diffractive states
     )
...
int EvtGen::ThePEGID(EvtId eid)
{
  int output(0);
  int id(EvtPDL::getStdHep(eid)),absid(abs(id)),ispin(absid%10),isgn(id/absid);
  // handle the easy cases
  if(absid<=8|| //quarks(+excited)
     (absid>=11&&absid<=18)|| // leptons(+excited)
     (absid>=21&&absid<=25)||(absid>=32&&absid<=37)|| // SM gauge bosons and higgs
     (absid>100&&absid<600&&(ispin%2==1&&ispin<9))|| // 1 1S0, 1 3S1 and 1 3P2 mesons are the same
     (absid>10100&&absid<10600&&(ispin==3))|| // 1 1P1 mesons are the same
     (absid>20100&&absid<20600&&(ispin==3))|| // 1 3P1 mesons are the same
     (absid>1000&&absid<6000&&ispin>=1&&ispin<=4)|| // lowest baryon multiplets and diquarks
     (absid>=100&&absid<=3000&&ispin==0) // mixed kaons and diffractive states
     )
    {output=id;}
  ...
  // 1 3p0 most same some different
    ...
    {
      // heavy and strange
      if(absid%1000>300&&absid!=10331){output=id;}
      // light
      else
	{
	  if(absid==10111||absid==10211)
	    {output=isgn*(absid-10000+9000000);}
  ...
  // 3s is a problem
  else if(absid>40100&&absid<40600&&ispin==3)
    {
      unsigned iq((absid%1000)/100);
      if(iq<3||iq==4)
	{output=isgn*(absid%1000+30000);}
      else if(iq==5)
	{if(absid!=40553){output=isgn*(absid%1000+120000);}}
    }
  else if((absid%1000)/10==55)
    {
		      ...
		      unsigned int & nbeforerad)
{
  ...
  do 
      ...
      if(damp)
	{
	  ...
	  do
		...
		{throw Exception() << "Negative probablity in EvtGen::randomDecay() " 
	      ...
	      --ntimes;
	      fail=prob<generator()->rnd()*prob_max;
	    }
	  while(ntimes>0&&fail);
	  if(ntimes==0)
	  ...
	  do
	    {
	      ...
	      dummy=dprob->getProbMax(prob)*generator()->rnd();
	...
	{throw Exception() << "Unknown type of EvtGen decayer in EvtGen::randomDecay()"
      ...
      if (decayer->getPHOTOS()||EvtRadCorr::alwaysRadCorr()){EvtRadCorr::doRadCorr(part);}

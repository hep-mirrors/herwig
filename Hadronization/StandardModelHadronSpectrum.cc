...
tcPDPtr StandardModelHadronSpectrum::lightestHadron(tcPDPtr ptr1, tcPDPtr ptr2,
#ifndef NDEBUG
				      tcPDPtr ptr3) const {
#else
				      tcPDPtr ) const {
  ...
  if (tit==_table.end()) 
    throw Exception() << "Could not find " 
		      ...
		      << " in _table. "
		      << "In StandardModelHadronSpectrum::lightestHadron()"
		      << Exception::eventerror;
  if(tit->second.empty())
    throw Exception() << "StandardModelHadronSpectrum::lightestHadron "
  ...
  if (candidate->mass() > 20*GeV 
    ...
    generator()->log() << "StandardModelHadronSpectrum::lightestHadron: "
...
StandardModelHadronSpectrum::hadronsBelowThreshold(Energy threshold, tcPDPtr ptr1,
				      tcPDPtr ptr2,
#ifndef NDEBUG
				      tcPDPtr ptr3) const {
#else
  ...
  if (tit==_table.end()) 
    throw Exception() << "Could not find " 
		      << ids.first << ' ' << ids.second 
		      << " in _table. "
		      << "In StandardModelHadronSpectrum::hadronsBelowThreshold()"
		      << Exception::eventerror;
  if(tit->second.empty())
    throw Exception() << "StandardModelHadronSpectrum::hadronsBelowThreshold() "
		      << "could not find any hadrons containing " 
		      << ptr1->id() << ' ' << ptr2->id() << '\n'
		      << tit->first.first << ' ' 
		      << tit->first.second << Exception::eventerror;
  ...
  while(hit!=tit->second.end()&&hit->mass<threshold) {
    ...
    if (candidate->mass() > 20*GeV 
	&& candidate->mass() < ptr1->constituentMass() + ptr2->constituentMass()) {
      generator()->log() << "StandardModelHadronSpectrum::hadronsBelowTheshold: "
			 << "chosen candidate " << candidate->PDGName() 
			 << " is lighter than its constituents "
			 << ptr1->PDGName() << ", " << ptr2->PDGName() << '\n'
			 << candidate->mass()/GeV << " < " << ptr1->constituentMass()/GeV
...
Energy StandardModelHadronSpectrum::massLightestBaryonPair(tcPDPtr ptr1, tcPDPtr ptr2) const {
...
double StandardModelHadronSpectrum::specialWeight(long id) const {
...
int StandardModelHadronSpectrum::signHadron(tcPDPtr idQ1, tcPDPtr idQ2, 

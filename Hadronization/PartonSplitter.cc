...
void PartonSplitter::colourSorted(PVector& tagged) {
  ...
  // enhancement
  colSingletSize.resize(0);
  colSingletm2.resize(0);
  ...
  while (int(left.size()) > 0){
    ...
    if ( !p->coloured() ) {
      ...
      nparts += (colSingletSize.empty()) ? 0 : colSingletSize.back();
      tempMom += p->momentum();
      Energy2 singletm2 = singletMass2(tempMom);
      // Store the number of particles and mass of the colour-singlet
      colSingletSize.push_back(nparts);
      colSingletm2.push_back(singletm2);
...
Energy2 PartonSplitter::singletMass2(const Lorentz5Momentum& pIn){
  Energy2 e2  = sqr(pIn.t());
  Energy2 px2 = sqr(pIn.x());
  Energy2 py2 = sqr(pIn.y());
  Energy2 pz2 = sqr(pIn.z());
...
double PartonSplitter::enhanceStrange(size_t i){
  ...
  // parton has
  auto const it = lower_bound(colSingletSize.begin(), colSingletSize.end(), i);

  // Get the index of the colourSinglet mass
  int indx = distance(colSingletSize.begin(), it);

  Energy2 mass2 = colSingletm2[indx];
  Energy2 m2 = _m0*_m0;
  ...
  if (_enhanceSProb == 1){
    ...
    double prob  = (20. < scale || scale < 0.) ? 0. : pow(_splitPwtSquark,scale);
    return prob;
  }
  // Exponential strangeness enhancement
  else if (_enhanceSProb == 2){
    double scale = double(m2/mass2);
    double prob  = (20. < scale || scale < 0.) ? 0. : exp(-scale);
    return prob;
  }
  else return _splitPwtSquark;
}

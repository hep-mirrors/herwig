...
METRP2to2::METRP2to2():_maxflavour(5),_process(0),_ndim(3), _planckmass(2000.0) {
    massOption(true ,0);
    massOption(false,0);
  }
...
Energy2 METRP2to2::scale() const {
  ...
  double invbcsq = pow(bccalc(UnitRemoval::InvE2 * s),-2);
  double qsq = - UnitRemoval::InvE2 * t;
  if(qsq > invbcsq) 
    return UnitRemoval::E2 * invbcsq;
  else  
    return UnitRemoval::E2 *  qsq;
...
METRP2to2::colourGeometries(tcDiagPtr diag) const {
  ...
  static const ColourLines cgggg[2]={ColourLines("1 4, -1 -4, 3 5, -3 -5"),
				     ColourLines("1 4, -1 -4, 3 5, -3 -5")};
  // colour lines for q g to q g
  static const ColourLines cqgqg[2]={ColourLines("1 4, 3 5, -3 -5"),
				     ColourLines("1 4, 3 5, -3 -5")};
  // colour lines for qbar g -> qbar g
  static const ColourLines cqbgqbg[2]={ColourLines("-1 -4, -3 -5, 3 5"),
				       ColourLines("-1 -4, -3 -5, 3 5")};
  // colour lines for q q -> q q 
  static const ColourLines cqqqq[2]={ColourLines("1 4,3 5"), 
				     ColourLines("1 4,3 5")};
  ...
  static const ColourLines cqqbqqb[2]={ColourLines("1 4,-3 -5"),
				       ColourLines("1 4,-3 -5")};
...
double METRP2to2::ME() const {
  Energy2 t(tHat()),s(sHat());
  return Any(UnitRemoval::InvE2 * s, UnitRemoval::InvE2 * t);
}

double METRP2to2::me2() const {
  std::cout << "ME = " << ME() << " s = " << sHat() * UnitRemoval::InvE2 << " t = " << tHat() * UnitRemoval::InvE2 << endl;
  return (ME() * 16 * sqr(Constants::pi));
}

  
/* Calculate the constant b_c which depends on s_hat and the number of extra dimensions */
double METRP2to2::bccalc(double s) const {  
  double n = double(_ndim);
  double resultbc = 0;  
  double M_D = _planckmass * 1000;
  return pow( (pow((4 * Constants::pi),(n/2.0) - 1.0) * s * Math::gamma(n/2.0) / (2 * pow(M_D,n+2.0)) ), 1.0/n);  
}
  

/* interp is a function for linear interpolation between two values in a table */
...
double METRP2to2::Any(double s, double t) const {
  double n = double(_ndim);
  double q = sqrt(-t);
  double M_D = _planckmass * 1000; //convert to MeV
  double bc = pow(pow((4 * Constants::pi),(n/2.0) - 1.0) * s * Math::gamma(n/2.0) / (2 * pow(M_D,n+2.0)),1.0/n);
  double Anyres = pow(s * fny(n,bc,q),2) * pow(bc,4);
  return Anyres;
}


/* the function F_n(q * bc), calculated using the tabulated values (x<20) and the asymptotic form (x>20) */
double METRP2to2::fny(double n, double bc, double q) const {
  double y = bc * q;
  double fnyres = 0;
...
double METRP2to2::fnyasympt(double n, double y) const {
...
/* fpoint contains tabulated values for the function F_n calculated using 8-point 
   Gauss-Legendre integration up to arguments x = 20 */
double METRP2to2::fpoint(int n, double x) const {   
  double fvalue;  
  ...
  if(x < 20) {     //make sure the value is within the tabulated range
    ...
    cout << "y out of range for datafile! " << endl; 
    fvalue = 0; 
  }

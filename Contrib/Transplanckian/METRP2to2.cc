...
METRP2to2::METRP2to2():_maxflavour(2),_process(0), _ndim(6),_planckmass(1500.0*GeV) {
    massOption(true ,0);
    massOption(false,0);
    doinitrun();
...
double METRP2to2::fpoint(double x) const {   
  assert( x < 20.0 );
  if( _ndim == 2 && x < 0.02 ) { assert( x < 0.02 );  return sqrt( sqr(-log(x/1.4)) + sqr(Constants::pi)/16 ); } 
  else { return (*_interpol)(x); }
}

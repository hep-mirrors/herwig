// -*- C++ -*-
//
// GnuplotOutput.cc is a part of Herwig++ - A multi-purpose Monte Carlo event generator
...
// functions of the GnuplotOutput class.
//
...
void GnuplotOutput::put (Histogram2Ptr histo, const Histogram2Options& out, const string& datachannel) {
  ...
  if (!histo->haveChannel("Herwig++")) return;
  ...
  gpfile << "set key samplen .7;" << endl
	 ...
	 << "Herwig{\\small ++}" << releaseversion << "}}' at graph .14,.94 back;" << endl
  ...
  gpfile << "set label 3 '{\\definecolor{Gray}{gray}{.5}\\textcolor{Gray}{\\tiny\\sffamily "
	 << svnversion << "}}' rotate by 90 at graph -.2,.4 left front;";
  ...
  histo->output(currentOStream(),"Herwig++");
  ...
  if (wantmult) {
    ...
    checkchannel << "Herwig++-" << (mrange.second + 1);

    bool havehigher = histo->haveChannel(checkchannel.str());

    if (!havehigher) {
      ostringstream multname; multname << "Herwig++-" << mrange.second;
	...
	ostringstream multname; multname << "Herwig++-" << i;
	if (i==mrange.second) {
	  histo->insertChannel("Herwig++-largerthanmax",histo->channel(multname.str()));
	} else {
	  if (histo->haveChannel(multname.str()))
	    histo->channel("Herwig++-largerthanmax") += histo->channel(multname.str());
	}
      }

      ostringstream key; key << "$+ \\ge" << (mrange.second-mrange.first) << "\\ {\\rm jets}$";
      putthem.push_back(make_pair("Herwig++-largerthanmax",key.str()));

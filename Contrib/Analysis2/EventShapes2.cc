// -*- C++ -*-
//
// EventShapes2.cc is a part of Herwig++ - A multi-purpose Monte Carlo event generator
...
// functions of the EventShapes2 class.
//
...
void EventShapes2::analyze(const tPVector & particles) {

  reset(particles);
  
  book(1.-thrust(), "OneMinusThrust");
  book(thrustMajor(), "ThrustMajor");
  book(thrustMinor(), "ThrustMinor");
  book(oblateness(), "Oblateness"); 
  book(CParameter(), "CParameter"); 
  book(DParameter(), "DParameter"); 
  book(sphericity(), "Sphericity");
  book(aplanarity(), "Aplanarity");
  book(planarity(), "Planarity"); 
  book(Mhigh2(), "MHigh");
  book(Mlow2(), "MLow"); 
  book(Mdiff2(), "MDiff"); 
  book(Bmax(), "BMax"); 
...
vector<double> EventShapes2::eigenvalues(const double T[3][3]) {
  ...
  if (4.*p*sqr(p) + 27.*sqr(q) > 0) {
    ...
    cerr << "EventShapes2::eigenvalues: found D > 0! \n"

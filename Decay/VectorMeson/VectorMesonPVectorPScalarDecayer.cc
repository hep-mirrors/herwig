...
void VectorMesonPVectorPScalarDecayer::doinit() {
  ...
  for(unsigned int ix=0;ix<_incoming.size();++ix) {
    ...
    tPDVector out = {getParticleData(_outgoingA[ix]),
...
DescribeClass<VectorMesonPVectorPScalarDecayer,DecayIntegrator2>
describeHerwigVectorMesonPVectorPScalarDecayer("Herwig::VectorMesonPVectorPScalarDecayer",
					       "HwVMDecay.so");
...
void VectorMesonPVectorPScalarDecayer::
...
double VectorMesonPVectorPScalarDecayer::me2(const int,const Particle & part,
					     const tPDVector & ,
  ...
  for(unsigned int ix=0;ix<3;++ix) {
    ...
    for(unsigned int iy=0;iy<3;++iy) {
      (*ME())(iy,ix,0)=pre*(p0dotpv*(_vectors[1][ix].dot(_vectors[0][iy]))-
			 epsdot*(_vectors[0][iy]*momenta[0]));
    }
  }

...
bool Evolver::timeLikeShower(tShowerParticlePtr particle)
{
  ...
  particle->showerKinematics()->splittingFn()->colourConnection(particle,showerProduct1,showerProduct2,false);
...
bool Evolver::spaceLikeShower(tShowerParticlePtr particle) {
  ...
  createBackwardBranching(particle,newParent,otherChild,
			  ...
			  particle->showerKinematics()->splittingFn()->interactionType());
  // update the history if needed
...
void Evolver::createBackwardBranching(ShowerParticlePtr part,
  ...
  part->showerKinematics()->splittingFn()->colourConnection(newParent,part,otherChild,true);
				   ...
				   Energy minmass)
{
  ...
  // In the case of splittings which involves coloured particles,
  // set properly the colour flow of the branching.
  particle->showerKinematics()->splittingFn()->
    colourConnection(particle,showerProduct1,showerProduct2,false);
  particle->addChild(showerProduct1);
  particle->addChild(showerProduct2);
  // update the history if needed

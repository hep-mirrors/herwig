...
void CKKWTree::removeBackChildren() {
...
bool CKKWTree::fixParents( HardBranchingPtr branch ){
...
Energy CKKWTree::lowestPt( int jetMeasureMode, Energy2 s ){
  // perfomr various checks
  assert( lowestpT_ && lowestpT_->children().size() == 2 && 
	  lowestpT_->children()[0] );


  Energy kt_softest = lowestpT_->children()[0]->pT();

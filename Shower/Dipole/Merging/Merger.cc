...
Merger::Merger() : MergerBase() ,
  ...
  theCurrentMaxLegs( -1 ) , xiRenME( 1. ) , xiFacME( 1. ) ,
  xiRenSh( 1. ) , xiFacSh( 1. ) , xiQSh( 1. ) , theGamma( 1. ) ,
  ee_ycut( -1 ) , pp_dcut( -1 ) , theSmearing( 0. ) ,
  therenormscale( -1.*GeV ) , theIRSafePT( 1.*GeV ) ,
...
pair<PVector , PVector> getInOut( NPtr Node ){
  ...
  for ( size_t i = 2;i<Node->nodeME()->mePartonData().size();i++ ){
    PPtr p  = Node->nodeME()->mePartonData()[i]->produceParticle( Node->nodeME()->lastMEMomenta()[i] );

...
// functions of the MEee2eePseudoScalar class.
//

#include "MEee2eePseudoScalar.h"
...
int MEee2eePseudoScalar::nDim() const {
  return 4;
}

Selector<const ColourLines *>
MEee2eePseudoScalar::colourGeometries(tcDiagPtr ) const {
...
IBPtr MEee2eePseudoScalar::clone() const {
  return new_ptr(*this);
}

IBPtr MEee2eePseudoScalar::fullclone() const {
  return new_ptr(*this);
}

void MEee2eePseudoScalar::doinit() {
  HwMEBase::doinit();
...
void MEee2eePseudoScalar::setKinematics() {
  HwMEBase::setKinematics();
}

void MEee2eePseudoScalar::getDiagrams() const {
  tcPDPtr ep = getParticleData(ParticleID::eplus );
  tcPDPtr em = getParticleData(ParticleID::eminus);
  tcPDPtr g  = getParticleData(ParticleID::gamma );
  for(long pid=111; pid<340; pid+=110) {
    tcPDPtr ps = getParticleData(pid);
    add(new_ptr((Tree2toNDiagram(4), em, g, g, ep, 1, em, 3, ep, 2, ps, -1)));
  }
}

CrossSection MEee2eePseudoScalar::dSigHatDR() const {
...
bool MEee2eePseudoScalar::generateKinematics(const double * r) {
  // roots
  Energy roots(sqrt(sHat()));
  // pseudoscalar mass
  Energy W = mePartonData()[4]->mass();
  Energy m = mePartonData()[0]->mass();
  Energy2 m2=sqr(m), W2=sqr(W);
  double beta = sqrt(1.-4.*m2/sHat());
  // initialise the jacobian
  jacobian(0.25*sqr(Constants::pi)/beta);
...
array<VectorWaveFunction,4> MEee2eePseudoScalar::electronCurrent() const {
  Energy m = mePartonData()[0]->mass();
  double mr2 = sqr(m/meMomenta()[0].t());
  array<VectorWaveFunction,4> output;
  ...
  output[0] = VectorWaveFunction(pGamma,gamma_, fact1*LorentzPolarizationVector(0.5*phase*(bb1-mr2/x)+
  ...
  output[3] = VectorWaveFunction(pGamma,gamma_, fact1*LorentzPolarizationVector(0.5/phase*(bb1-mr2/x)+ 
										b1*(mr2+bb1*x)*cos(phi1_)*sqr(cHalf1_)/(1.-x),
										0.5/phase*II*(bb1-mr2/x)+ 
										b1*(mr2+bb1*x)*sqr(cHalf1_)*sin(phi1_)/(1.-x),
										mr2*(1.+b1-(1.+b)/x)*cHalf1_/sHalf1_/(1.-x) + 
										-b1*(mr2+bb1*x)*cHalf1_*sHalf1_/(1.-x),0));
  output[1] = VectorWaveFunction(pGamma,gamma_, fact2*LorentzPolarizationVector(-(1.+b-(1.+b1)*x)*cHalf1_,
										II*(1.+b-(1.+b1)*x)*cHalf1_,
										(1.+b-(1.+b1)*x)*sHalf1_/phase,
										(1.+b+(1.+b1)*x)*sHalf1_/phase));
  output[2] = VectorWaveFunction(pGamma,gamma_, fact2*LorentzPolarizationVector((1.+b-(1.+b1)*x)*cHalf1_,
										II*(1.+b-(1.+b1)*x)*cHalf1_,
...
array<VectorWaveFunction,4> MEee2eePseudoScalar::positronCurrent() const {
  ...
  output[0] = VectorWaveFunction(pGamma,gamma_, fact1*LorentzPolarizationVector(0.5*(bb2-mr2/x) +
  ...
  output[3] = VectorWaveFunction(pGamma,gamma_, fact1*LorentzPolarizationVector(0.5*(bb2-mr2/x) + 
										b2/phase*(mr2+bb2*x)*cos(phi2_)*
										sqr(sHalf2_)/(1.-x),
										-0.5*II*(bb2-mr2/x) + 
										b2/phase*(mr2+bb2*x)*sin(phi2_)*
  ...
  output[1] = VectorWaveFunction(pGamma,gamma_, fact2*LorentzPolarizationVector((1.+b-(1.+b2)*x)*sHalf2_/phase,
										II*(1.+b-(1.+b2)*x)*sHalf2_/phase,
										(1.+b-(1.+b2)*x)*cHalf2_,
										-(1.+b+(1.+b2)*x)*cHalf2_));
  output[2] = VectorWaveFunction(pGamma,gamma_, fact2*LorentzPolarizationVector(-phase*(1.+b-(1.+b2)*x)*sHalf2_,
...
double MEee2eePseudoScalar::me2() const {
  // cerr << "testing in matrix element";
  // for(unsigned int ix=0;ix<5;++ix)
  //   cerr << mePartonData()[ix]->PDGName() << " " << meMomenta()[ix]/GeV << "\n";
  // calculate the leptonic currents
  array<VectorWaveFunction,4> eCurrent = electronCurrent();
  array<VectorWaveFunction,4> pCurrent = positronCurrent();
  // form factor
  int iloc = mePartonData()[4]->id()/100 -1;
  InvEnergy form = F00_[iloc]/(1.-t1_/LambdaP2_[iloc])/
    (1.-t2_/LambdaP2_[iloc]);
  // calculate the matrix element
  double output(0.);
  Lorentz5Momentum pG1 = meMomenta()[0]-meMomenta()[2];
  Lorentz5Momentum pG2 = meMomenta()[1]-meMomenta()[3];
  Energy rs = sqrt(sHat());
  for(unsigned int ih1=0;ih1<4;++ih1) {
    auto v1 = Helicity::epsilon(eCurrent[ih1].wave(),pG1,pG2);
    for(unsigned int ih2=0;ih2<4;++ih2) {
      Complex amp = form*(v1*pCurrent[ih2].wave())*rs*UnitRemoval::InvE2;
      output += norm(amp);
    }
  }
  // coupling factors
...
void MEee2eePseudoScalar::persistentOutput(PersistentOStream & os) const {
  os << FFPVertex_ << gamma_
     << ounit(Q2_1min_,GeV2) << ounit(Q2_1max_,GeV2)
     << ounit(Q2_2min_,GeV2) << ounit(Q2_2max_,GeV2)
     << ounit(F00_,1./GeV) << ounit(LambdaP2_,GeV2);
}

void MEee2eePseudoScalar::persistentInput(PersistentIStream & is, int) {
  is >> FFPVertex_ >> gamma_
     >> iunit(Q2_1min_,GeV2) >> iunit(Q2_1max_,GeV2)
     >> iunit(Q2_2min_,GeV2) >> iunit(Q2_2max_,GeV2)
     >> iunit(F00_,1./GeV) >> iunit(LambdaP2_,GeV2);
}

//The following static variable is needed for the type
// description system in ThePEG.
DescribeClass<MEee2eePseudoScalar,HwMEBase>
  describeHerwigMEee2eePseudoScalar("Herwig::MEee2eePseudoScalar",
				    "HwMEGammaGamma.so");

void MEee2eePseudoScalar::Init() {

  static ClassDocumentation<MEee2eePseudoScalar> documentation
    ("There is no documentation for the MEee2eePseudoScalar class");

  static Parameter<MEee2eePseudoScalar,Energy2> interfaceQ2_1Min
    ("Q2_1Min",
     "The minimum value of Q2 for the off-shell photon from the electron",
     &MEee2eePseudoScalar::Q2_1min_, GeV2, 0.*GeV2, 0.0*GeV2, Constants::MaxEnergy2,
     false, false, Interface::limited);

  static Parameter<MEee2eePseudoScalar,Energy2> interfaceQ2_1Max
    ("Q2_1Max",
     "The maximum value of Q2 for the off-shell photon from the electron",
     &MEee2eePseudoScalar::Q2_1max_, GeV2, 0.*GeV2, 0.0*GeV2, Constants::MaxEnergy2,
     false, false, Interface::limited);

  static Parameter<MEee2eePseudoScalar,Energy2> interfaceQ2_2Min
    ("Q2_2Min",
     "The minimum value of Q2 for the off-shell photon from the electron",
     &MEee2eePseudoScalar::Q2_2min_, GeV2, 0.*GeV2, 0.0*GeV2, Constants::MaxEnergy2,
     false, false, Interface::limited);

  static Parameter<MEee2eePseudoScalar,Energy2> interfaceQ2_2Max
    ("Q2_2Max",
     "The maximum value of Q2 for the off-shell photon from the electron",
     &MEee2eePseudoScalar::Q2_2max_, GeV2, 0.*GeV2, 0.0*GeV2, Constants::MaxEnergy2,
     false, false, Interface::limited);

  static ParVector<MEee2eePseudoScalar,InvEnergy> interfaceF00
    ("F00",
     "The form factor at zero momentum transfer",
     &MEee2eePseudoScalar::F00_, 1./GeV, 3, 1./GeV, 0./GeV, 100./GeV,
     false, false, Interface::limited);

  static ParVector<MEee2eePseudoScalar,Energy2> interfaceLambdaP2
    ("LambdaP2",
     "The square of the pole mass for the form factor",
     &MEee2eePseudoScalar::LambdaP2_, GeV2, 3, 1.0*GeV2, 0.0*GeV2, 10.0*GeV2,
     false, false, Interface::limited);

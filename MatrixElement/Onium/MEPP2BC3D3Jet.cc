...
double MEPP2BC3D3Jet::me2() const {
  ...
  if(mePartonData()[0]->id()==ParticleID::g) {
	...
	for(unsigned int ih4=0;ih4<2;++ih4) {
	  ...
	  complex<Energy2> dot27 = vec1*rescaledMomenta()[0];
	  for(unsigned int ih3=0;ih3<7;++ih3) {
	    LorentzVector<complex<Energy2> > vec5 = ten3[ih3].wave().dot(rescaledMomenta()[0],0).preDot(rescaledMomenta()[0]);
	    LorentzVector<complex<Energy2> > vec6 = ten3[ih3].wave().dot(rescaledMomenta()[0],0).preDot(rescaledMomenta()[1]);
	    LorentzVector<complex<Energy2> > vec8 = ten3[ih3].wave().dot(rescaledMomenta()[1],0).preDot(rescaledMomenta()[1]);
	    complex<Energy3> dot4 = vec1*vec5;
	    complex<Energy3> dot5 = vec1*vec6;
	    complex<Energy3> dot6 = vec1*vec8;
	    complex<Energy4> dot15 = vec3*vec5;
	    complex<Energy4> dot16 = vec3*vec6;
	    complex<Energy4> dot17 = vec3*vec8;
	    complex<Energy3> dot18 = vec5*rescaledMomenta()[0];
	    complex<Energy3> dot20 = vec5*rescaledMomenta()[1];
	    complex<Energy3> dot22 = vec6*rescaledMomenta()[1];
	    complex<Energy3> dot24 = vec8*rescaledMomenta()[1];
	    for(unsigned int ih1=0;ih1<2;++ih1) {
	      ...
	      LorentzPolarizationVectorE vec7 = ten3[ih3].wave().dot(rescaledMomenta()[0],0).preDot(g1[ih1].wave());
	      LorentzPolarizationVectorE vec9 = ten3[ih3].wave().dot(rescaledMomenta()[1],0).preDot(g1[ih1].wave());
	      complex<Energy4> dot7 = vec4*vec5;
	      complex<Energy4> dot8 = vec4*vec6;
	      complex<Energy4> dot9 = vec4*vec8;
	      complex<Energy2> dot10 = vec1*vec7;
	      complex<Energy2> dot11 = vec1*vec9;
	      complex<Energy3> dot12 = vec2*vec5;
	      complex<Energy3> dot13 = vec2*vec6;
	      complex<Energy3> dot14 = vec2*vec8;
	      complex<Energy2> dot19 = vec2*rescaledMomenta()[0];
	      complex<Energy2> dot21 = vec5*g1[ih1].wave();
	      complex<Energy2> dot23 = vec6*g1[ih1].wave();
	      complex<Energy2> dot25 = vec8*g1[ih1].wave();
	      complex<Energy> dot26 = vec1*g1[ih1].wave();
	      // diagrams
	...
	for(unsigned int ih4=0;ih4<2;++ih4) {
	  ...
	  complex<Energy2> dot13 = vec1*rescaledMomenta()[0];
	  for(unsigned int ih3=0;ih3<7;++ih3) {
	    LorentzVector<complex<Energy2> > vec5 = ten3[ih3].wave().dot(rescaledMomenta()[0],0).preDot(rescaledMomenta()[0]);
	    LorentzVector<complex<Energy2> > vec6 = ten3[ih3].wave().dot(rescaledMomenta()[0],0).preDot(rescaledMomenta()[1]);
	    LorentzVector<complex<Energy2> > vec8 = ten3[ih3].wave().dot(rescaledMomenta()[1],0).preDot(rescaledMomenta()[1]);
	    ...
	    complex<Energy3> dot5 = vec1*vec6;
	    complex<Energy3> dot6 = vec1*vec8;
	    complex<Energy3> dot11 = vec5*rescaledMomenta()[0];
	    complex<Energy3> dot12 = vec5*rescaledMomenta()[1];
	    complex<Energy3> dot15 = vec6*rescaledMomenta()[1];
	    complex<Energy4> dot23 = vec3*vec5;
	    complex<Energy4> dot24 = vec3*vec6;
	    complex<Energy4> dot25 = vec3*vec8;
	    complex<Energy3> dot27 = vec8*rescaledMomenta()[1];
	    for(unsigned int ih1=0;ih1<2;++ih1) {
	      complex<Energy> dot1 = rescaledMomenta()[1]*g1[ih1].wave();
	      complex<Energy> dot2 = rescaledMomenta()[3]*g1[ih1].wave();
	      ...
	      complex<Energy2> dot14 = vec5*g1[ih1].wave();
	      complex<Energy2> dot16 = vec6*g1[ih1].wave();
	      complex<Energy2> dot17 = vec8*g1[ih1].wave();
	      complex<Energy2> dot18 = vec1*vec7;
	      complex<Energy2> dot19 = vec1*vec9;
	      complex<Energy3> dot20 = vec2*vec5;
	      complex<Energy3> dot21 = vec2*vec6;
	      complex<Energy3> dot22 = vec2*vec8;
	      complex<Energy2> dot26 = vec2*rescaledMomenta()[0];
	      // diagrams
  ...
  // q qbar process
	...
	complex<Energy2> dot25 = vec1*rescaledMomenta()[3];
	for(unsigned int ih3=0;ih3<7;++ih3) {
	  LorentzVector<complex<Energy2> > vec5 = ten3[ih3].wave().dot(rescaledMomenta()[0],0).preDot(rescaledMomenta()[0]);
	  LorentzVector<complex<Energy2> > vec6 = ten3[ih3].wave().dot(rescaledMomenta()[0],0).preDot(rescaledMomenta()[1]);
	  LorentzVector<complex<Energy2> > vec8 = ten3[ih3].wave().dot(rescaledMomenta()[1],0).preDot(rescaledMomenta()[1]);
	  complex<Energy3> dot4 = vec1*vec5;
	  complex<Energy3> dot5 = vec1*vec6;
	  complex<Energy3> dot7 = vec1*vec8;
	  complex<Energy4> dot12 = vec3*vec5;
	  complex<Energy4> dot13 = vec3*vec6;
	  complex<Energy4> dot14 = vec3*vec8;
	  complex<Energy3> dot15 = vec5*rescaledMomenta()[0];
	  complex<Energy3> dot17 = vec5*rescaledMomenta()[1];
	  complex<Energy3> dot19 = vec6*rescaledMomenta()[1];
	  complex<Energy3> dot21 = vec8*rescaledMomenta()[1];
	  for(unsigned int ih4=0;ih4<2;++ih4) {
	    ...
	    LorentzPolarizationVectorE vec7 = ten3[ih3].wave().dot(rescaledMomenta()[0],0).preDot(g4[ih4].wave());
	    LorentzPolarizationVectorE vec9 = ten3[ih3].wave().dot(rescaledMomenta()[1],0).preDot(g4[ih4].wave());
	    complex<Energy2> dot6 = vec1*vec7;
	    complex<Energy2> dot8 = vec1*vec9;
	    complex<Energy3> dot9 = vec2*vec5;
	    complex<Energy3> dot10 = vec2*vec6;
	    complex<Energy3> dot11 = vec2*vec8;
	    complex<Energy2> dot16 = vec2*rescaledMomenta()[3];
	    complex<Energy2> dot18 = vec5*g4[ih4].wave();
	    complex<Energy2> dot20 = vec6*g4[ih4].wave();
	    complex<Energy2> dot22 = vec8*g4[ih4].wave();
	    complex<Energy4> dot23 = vec4*vec5;
	    complex<Energy> dot24 = vec1*g4[ih4].wave();
	    complex<Energy4> dot26 = vec4*vec6;
	    complex<Energy4> dot27 = vec4*vec8;
	    // diagrams

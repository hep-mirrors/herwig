...
double MEQCD2to2::qqbar2ggME(vector<SpinorWaveFunction> & q,
  ...
  for(unsigned int ihel1=0;ihel1<2;++ihel1) { 
	...
	for(unsigned int ohel2=0;ohel2<2;++ohel2) {
	  // first t-channel diagram
	  inters=_qqgvertex->evaluate(mt,5,q[ihel1].getParticle()->CC(),
				      q[ihel1],g1[ohel1]);
...
double MEQCD2to2::qg2qgME(vector<SpinorWaveFunction> & qin,
  ...
  for(unsigned int ihel1=0;ihel1<2;++ihel1) { 
    for(unsigned int ihel2=0;ihel2<2;++ihel2) {
      inters=_qqgvertex->evaluate(mt,5,qin[ihel1].getParticle()->CC(),
				  qin[ihel1],g2[ihel2]);
      for(unsigned int ohel1=0;ohel1<2;++ohel1) { 
	for(unsigned int ohel2=0;ohel2<2;++ohel2) {
	  // s-channel diagram
	  ...
	  // first t-channel
	  inters2=_qqgvertex->evaluate(mt,5,qin[ihel1].getParticle()->CC(),
				       qin[ihel1],g4[ohel2]);

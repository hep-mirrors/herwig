...
#include "DC_Hjjj_CC_qqB.h"

using namespace Herwig;

IBPtr DC_Hjjj_CC_qqB::clone() const {
  return new_ptr(*this);
}
IBPtr DC_Hjjj_CC_qqB::fullclone() const {
  return new_ptr(*this);
}

vector<DiagPtr> DC_Hjjj_CC_qqB::getDiagrams() const {
  ...
  for (int genU = 0; genU < 2; genU++) {
    for (int genL = 0; genL < 2; genL++) {
      ...
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(5), qd, Wplus, Wplus, kd, kdB, 1, qdC, 3, kdBC, 4, g, 2, h0, -1)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(4), qd, Wplus, Wplus, kdB, 3, kdBC, 1, qdC, 5, kdBC, 5, g, 2, h0, -2)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(5), qd, qd, Wplus, Wplus, kdB, 2, qdC, 4, kdBC, 1, g, 3, h0, -3)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(4), qd, Wplus, Wplus, kdB, 1, qdC, 5, qdC, 3, kdBC, 5, g, 2, h0, -4)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(5), qu, Wminus, Wminus, ku, kuB, 1, quC, 3, kuBC, 4, g, 2, h0, -5)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(4), qu, Wminus, Wminus, kuB, 3, kuBC, 1, quC, 5, kuBC, 5, g, 2, h0, -6)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(5), qu, qu, Wminus, Wminus, kuB, 2, quC, 4, kuBC, 1, g, 3, h0, -7)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(4), qu, Wminus, Wminus, kuB, 1, quC, 5, quC, 3, kuBC, 5, g, 2, h0, -8)));
    }
  }
...
Selector<const ColourLines *> DC_Hjjj_CC_qqB::colourGeometries(tcDiagPtr diag) const {
  // colour lines for DC_Hjjj_CC_qqB

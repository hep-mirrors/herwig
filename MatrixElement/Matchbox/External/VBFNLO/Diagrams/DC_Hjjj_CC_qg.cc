...
#include "DC_Hjjj_CC_qg.h"

using namespace Herwig;

IBPtr DC_Hjjj_CC_qg::clone() const {
  return new_ptr(*this);
}
IBPtr DC_Hjjj_CC_qg::fullclone() const {
  return new_ptr(*this);
}

vector<DiagPtr> DC_Hjjj_CC_qg::getDiagrams() const {
  ...
  for (int genU = 0; genU < 2; genU++) {
    for (int genL = 0; genL < 2; genL++) {
      ...
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(5), qd, Wplus, Wplus, kuC, g, 1, qdC, 4, kuC, 3, kuB, 2, h0, -1)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(5), qd, Wplus, Wplus, kuB, g, 1, qdC, 3, kuC, 4, kuB, 2, h0, -2)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(5), qu, Wminus, Wminus, kdC, g, 1, quC, 4, kdC, 3, kdB, 2, h0, -3)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(5), qu, Wminus, Wminus, kdB, g, 1, quC, 3, kdC, 4, kdB, 2, h0, -4)));
    }
  }
...
Selector<const ColourLines *> DC_Hjjj_CC_qg::colourGeometries(tcDiagPtr diag) const {
  // colour lines for DC_Hjjj_CC_qg

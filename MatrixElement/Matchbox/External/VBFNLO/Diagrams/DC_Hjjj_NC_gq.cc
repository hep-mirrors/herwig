...
#include "DC_Hjjj_NC_gq.h"

using namespace Herwig;

IBPtr DC_Hjjj_NC_gq::clone() const {
  return new_ptr(*this);
}
IBPtr DC_Hjjj_NC_gq::fullclone() const {
  return new_ptr(*this);
}

vector<DiagPtr> DC_Hjjj_NC_gq::getDiagrams() const {
  ...
  for (int genU = 0; genU < 2; genU++) {
    for (int genL = 0; genL < 2; genL++) {
      ...
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(5), g, qdB, Z0, Z0, kd, 1, qd, 4, kd, 2, qdB, 3, h0, -1)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(5), g, qd, Z0, Z0, kd, 2, qd, 4, kd, 1, qdB, 3, h0, -2)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(5), g, quB, Z0, Z0, kd, 1, qu, 4, kd, 2, quB, 3, h0, -3)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(5), g, qu, Z0, Z0, kd, 2, qu, 4, kd, 1, quB, 3, h0, -4)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(5), g, qdB, Z0, Z0, ku, 1, qd, 4, ku, 2, qdB, 3, h0, -5)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(5), g, qd, Z0, Z0, ku, 2, qd, 4, ku, 1, qdB, 3, h0, -6)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(5), g, quB, Z0, Z0, ku, 1, qu, 4, ku, 2, quB, 3, h0, -7)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(5), g, qu, Z0, Z0, ku, 2, qu, 4, ku, 1, quB, 3, h0, -8)));
    }
  }
...
Selector<const ColourLines *> DC_Hjjj_NC_gq::colourGeometries(tcDiagPtr diag) const {
  // colour lines for DC_Hjjj_NC_gq

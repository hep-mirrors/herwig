...
#include "DC_Hjj_NC_qBq.h"

using namespace Herwig;

IBPtr DC_Hjj_NC_qBq::clone() const {
  return new_ptr(*this);
}
IBPtr DC_Hjj_NC_qBq::fullclone() const {
  return new_ptr(*this);
}

vector<DiagPtr> DC_Hjj_NC_qBq::getDiagrams() const {
  ...
  for (int genU = 0; genU < 2; genU++) {
    for (int genL = 0; genL < 2; genL++) {
      ...
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(4), qdB, Z0, Z0, kd, 1, qdB, 3, kd, 2, h0, -1)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(4), qdB, Z0, Z0, ku, 1, qdB, 3, ku, 2, h0, -2)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(4), quB, Z0, Z0, kd, 1, quB, 3, kd, 2, h0, -3)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(4), quB, Z0, Z0, ku, 1, quB, 3, ku, 2, h0, -4)));
    }
  }
...
Selector<const ColourLines *> DC_Hjj_NC_qBq::colourGeometries(tcDiagPtr diag) const {
  // colour lines for DC_Hjj_NC_qBq

...
#include "DC_Hjjj_NC_qg.h"

using namespace Herwig;

IBPtr DC_Hjjj_NC_qg::clone() const {
  return new_ptr(*this);
}
IBPtr DC_Hjjj_NC_qg::fullclone() const {
  return new_ptr(*this);
}

vector<DiagPtr> DC_Hjjj_NC_qg::getDiagrams() const {
  ...
  for (int genU = 0; genU < 2; genU++) {
    for (int genL = 0; genL < 2; genL++) {
      ...
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(5), qd, Z0, Z0, kd, g, 1, qd, 4, kd, 3, kdB, 2, h0, -1)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(5), qd, Z0, Z0, kdB, g, 1, qd, 3, kd, 4, kdB, 2, h0, -2)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(5), qd, Z0, Z0, ku, g, 1, qd, 4, ku, 3, kuB, 2, h0, -3)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(5), qd, Z0, Z0, kuB, g, 1, qd, 3, ku, 4, kuB, 2, h0, -4)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(5), qu, Z0, Z0, kd, g, 1, qu, 4, kd, 3, kdB, 2, h0, -5)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(5), qu, Z0, Z0, kdB, g, 1, qu, 3, kd, 4, kdB, 2, h0, -6)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(5), qu, Z0, Z0, ku, g, 1, qu, 4, ku, 3, kuB, 2, h0, -7)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(5), qu, Z0, Z0, kuB, g, 1, qu, 3, ku, 4, kuB, 2, h0, -8)));
    }
  }
...
Selector<const ColourLines *> DC_Hjjj_NC_qg::colourGeometries(tcDiagPtr diag) const {
  // colour lines for DC_Hjjj_NC_qg

...
#include "DC_Hjjj_CC_qBqB.h"

using namespace Herwig;

IBPtr DC_Hjjj_CC_qBqB::clone() const {
  return new_ptr(*this);
}
IBPtr DC_Hjjj_CC_qBqB::fullclone() const {
  return new_ptr(*this);
}

vector<DiagPtr> DC_Hjjj_CC_qBqB::getDiagrams() const {
  ...
  for (int genU = 0; genU < 2; genU++) {
    for (int genL = 0; genL < 2; genL++) {
      ...
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(5), qdB, Wminus, Wminus, ku, kuB, 1, qdBC, 3, kuBC, 4, g, 2, h0, -1)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(4), qdB, Wminus, Wminus, kuB, 3, kuBC, 1, qdBC, 5, kuBC, 5, g, 2, h0, -2)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(5), qdB, qdB, Wminus, Wminus, kuB, 2, qdBC, 4, kuBC, 1, g, 3, h0, -3)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(4), qdB, Wminus, Wminus, kuB, 1, qdBC, 5, qdBC, 3, kuBC, 5, g, 2, h0, -4)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(5), quB, Wplus, Wplus, kd, kdB, 1, quBC, 3, kdBC, 4, g, 2, h0, -5)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(4), quB, Wplus, Wplus, kdB, 3, kdBC, 1, quBC, 5, kdBC, 5, g, 2, h0, -6)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(5), quB, quB, Wplus, Wplus, kdB, 2, quBC, 4, kdBC, 1, g, 3, h0, -7)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(4), quB, Wplus, Wplus, kdB, 1, quBC, 5, quBC, 3, kdBC, 5, g, 2, h0, -8)));
    }
  }
...
Selector<const ColourLines *> DC_Hjjj_CC_qBqB::colourGeometries(tcDiagPtr diag) const {
  // colour lines for DC_Hjjj_CC_qBqB

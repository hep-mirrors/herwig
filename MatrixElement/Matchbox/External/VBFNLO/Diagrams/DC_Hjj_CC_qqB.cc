...
#include "DC_Hjj_CC_qqB.h"

using namespace Herwig;

IBPtr DC_Hjj_CC_qqB::clone() const {
  return new_ptr(*this);
}
IBPtr DC_Hjj_CC_qqB::fullclone() const {
  return new_ptr(*this);
}

vector<DiagPtr> DC_Hjj_CC_qqB::getDiagrams() const {
  ...
  for (int genU = 0; genU < 2; genU++) {
    for (int genL = 0; genL < 2; genL++) {
      ...
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(4), qd, Wplus, Wplus, kdB, 1, qdC, 3, kdBC, 2, h0, -1)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(4), qu, Wminus, Wminus, kuB, 1, quC, 3, kuBC, 2, h0, -2)));
    }
  }
...
Selector<const ColourLines *> DC_Hjj_CC_qqB::colourGeometries(tcDiagPtr diag) const {
  // colour lines for DC_Hjj_CC_qqB

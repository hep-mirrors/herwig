...
#include "DC_Hjj_NC_qqB.h"

using namespace Herwig;

IBPtr DC_Hjj_NC_qqB::clone() const {
  return new_ptr(*this);
}
IBPtr DC_Hjj_NC_qqB::fullclone() const {
  return new_ptr(*this);
}

vector<DiagPtr> DC_Hjj_NC_qqB::getDiagrams() const {
  ...
  for (int genU = 0; genU < 2; genU++) {
    for (int genL = 0; genL < 2; genL++) {
      ...
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(4), qd, Z0, Z0, kdB, 1, qd, 3, kdB, 2, h0, -1)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(4), qd, Z0, Z0, kuB, 1, qd, 3, kuB, 2, h0, -2)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(4), qu, Z0, Z0, kdB, 1, qu, 3, kdB, 2, h0, -3)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(4), qu, Z0, Z0, kuB, 1, qu, 3, kuB, 2, h0, -4)));
    }
  }
...
Selector<const ColourLines *> DC_Hjj_NC_qqB::colourGeometries(tcDiagPtr diag) const {
  // colour lines for DC_Hjj_NC_qqB

...
#include "DC_Hjj_CC_qBq.h"

using namespace Herwig;

IBPtr DC_Hjj_CC_qBq::clone() const {
  return new_ptr(*this);
}
IBPtr DC_Hjj_CC_qBq::fullclone() const {
  return new_ptr(*this);
}

vector<DiagPtr> DC_Hjj_CC_qBq::getDiagrams() const {
  ...
  for (int genU = 0; genU < 2; genU++) {
    for (int genL = 0; genL < 2; genL++) {
      ...
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(4), quB, Wplus, Wplus, ku, 1, quBC, 3, kuC, 2, h0, -2)));
    }
  }
...
Selector<const ColourLines *> DC_Hjj_CC_qBq::colourGeometries(tcDiagPtr diag) const {
  // colour lines for DC_Hjj_CC_qBq

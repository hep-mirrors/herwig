...
#include "DC_Hjjj_CC_qq.h"

using namespace Herwig;

IBPtr DC_Hjjj_CC_qq::clone() const {
  return new_ptr(*this);
}
IBPtr DC_Hjjj_CC_qq::fullclone() const {
  return new_ptr(*this);
}

vector<DiagPtr> DC_Hjjj_CC_qq::getDiagrams() const {
  ...
  for (int genU = 0; genU < 2; genU++) {
    for (int genL = 0; genL < 2; genL++) {
      ...
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(5), qd, Wplus, Wplus, kuB, ku, 1, qdC, 3, kuC, 4, g, 2, h0, -1)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(4), qd, Wplus, Wplus, ku, 3, kuC, 1, qdC, 5, kuC, 5, g, 2, h0, -2)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(5), qd, qd, Wplus, Wplus, ku, 2, qdC, 4, kuC, 1, g, 3, h0, -3)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(4), qd, Wplus, Wplus, ku, 1, qdC, 5, qdC, 3, kuC, 5, g, 2, h0, -4)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(5), qu, Wminus, Wminus, kdB, kd, 1, quC, 3, kdC, 4, g, 2, h0, -5)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(4), qu, Wminus, Wminus, kd, 3, kdC, 1, quC, 5, kdC, 5, g, 2, h0, -6)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(5), qu, qu, Wminus, Wminus, kd, 2, quC, 4, kdC, 1, g, 3, h0, -7)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(4), qu, Wminus, Wminus, kd, 1, quC, 5, quC, 3, kdC, 5, g, 2, h0, -8)));
    }
  }
...
Selector<const ColourLines *> DC_Hjjj_CC_qq::colourGeometries(tcDiagPtr diag) const {
  // colour lines for DC_Hjjj_CC_qq

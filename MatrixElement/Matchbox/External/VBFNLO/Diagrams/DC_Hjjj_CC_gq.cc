...
#include "DC_Hjjj_CC_gq.h"

using namespace Herwig;

IBPtr DC_Hjjj_CC_gq::clone() const {
  return new_ptr(*this);
}
IBPtr DC_Hjjj_CC_gq::fullclone() const {
  return new_ptr(*this);
}

vector<DiagPtr> DC_Hjjj_CC_gq::getDiagrams() const {
  ...
  for (int genU = 0; genU < 2; genU++) {
    for (int genL = 0; genL < 2; genL++) {
      ...
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(5), g, qdBC, Wplus, Wplus, ku, 1, qdC, 4, kuC, 2, qdB, 3, h0, -3)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(5), g, qd, Wplus, Wplus, ku, 2, qdC, 4, kuC, 1, qdB, 3, h0, -4)));
    }
  }
...
Selector<const ColourLines *> DC_Hjjj_CC_gq::colourGeometries(tcDiagPtr diag) const {
  // colour lines for DC_Hjjj_CC_gq

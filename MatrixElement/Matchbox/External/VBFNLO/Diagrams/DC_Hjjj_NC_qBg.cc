...
#include "DC_Hjjj_NC_qBg.h"

using namespace Herwig;

IBPtr DC_Hjjj_NC_qBg::clone() const {
  return new_ptr(*this);
}
IBPtr DC_Hjjj_NC_qBg::fullclone() const {
  return new_ptr(*this);
}

vector<DiagPtr> DC_Hjjj_NC_qBg::getDiagrams() const {
  ...
  for (int genU = 0; genU < 2; genU++) {
    for (int genL = 0; genL < 2; genL++) {
      ...
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(5), qdB, Z0, Z0, kd, g, 1, qdB, 4, kd, 3, kdB, 2, h0, -1)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(5), qdB, Z0, Z0, kdB, g, 1, qdB, 3, kd, 4, kdB, 2, h0, -2)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(5), qdB, Z0, Z0, ku, g, 1, qdB, 4, ku, 3, kuB, 2, h0, -3)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(5), qdB, Z0, Z0, kuB, g, 1, qdB, 3, ku, 4, kuB, 2, h0, -4)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(5), quB, Z0, Z0, kd, g, 1, quB, 4, kd, 3, kdB, 2, h0, -5)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(5), quB, Z0, Z0, kdB, g, 1, quB, 3, kd, 4, kdB, 2, h0, -6)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(5), quB, Z0, Z0, ku, g, 1, quB, 4, ku, 3, kuB, 2, h0, -7)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(5), quB, Z0, Z0, kuB, g, 1, quB, 3, ku, 4, kuB, 2, h0, -8)));
    }
  }
...
Selector<const ColourLines *> DC_Hjjj_NC_qBg::colourGeometries(tcDiagPtr diag) const {
  // colour lines for DC_Hjjj_NC_qBg

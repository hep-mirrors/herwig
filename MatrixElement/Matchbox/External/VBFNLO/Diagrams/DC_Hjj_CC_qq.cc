...
#include "DC_Hjj_CC_qq.h"

using namespace Herwig;

IBPtr DC_Hjj_CC_qq::clone() const {
  return new_ptr(*this);
}
IBPtr DC_Hjj_CC_qq::fullclone() const {
  return new_ptr(*this);
}

vector<DiagPtr> DC_Hjj_CC_qq::getDiagrams() const {
  ...
  for (int genU = 0; genU < 2; genU++) {
    for (int genL = 0; genL < 2; genL++) {
      ...
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(4), qd, Wplus, Wplus, ku, 1, qdC, 3, kuC, 2, h0, -1)));
      allPossibleDiagrams.push_back(new_ptr((Tree2toNDiagram(4), qu, Wminus, Wminus, kd, 1, quC, 3, kdC, 2, h0, -2)));
    }
  }
...
Selector<const ColourLines *> DC_Hjj_CC_qq::colourGeometries(tcDiagPtr diag) const {
  // colour lines for DC_Hjj_CC_qq

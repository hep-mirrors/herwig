// -*- C++ -*-
//
// arHistogram.cc is a part of Herwig++ - A multi-purpose Monte Carlo event generator
...
// functions of the arHistogram class.
//
...
NoPIOClassDescription<arHistogram> arHistogram::initarHistogram;
// Definition of the static class description member.
void arHistogram::Init() {

  static ClassDocumentation<arnold::arHistogram> documentation
    ("The arHistogram class implements a simple histogram include data"
     " points for comparision with experimental results.");

}

void arHistogram::topdrawOutput(ostream & out,
			      unsigned int flags,
			      string colour,
  ...
  using namespace arHistogramOptions;
...
double arHistogram::dataNorm() const {
  double norm(0.0);
  if (_havedata) {
    const unsigned int lastDataBinIndx = _bins.size()-2;
    for(unsigned int ix=1; ix<=lastDataBinIndx; ++ix) {
...
unsigned int arHistogram::visibleEntries() const {
  //unsigned int numPoints(0);
  double counter = 0;
  const unsigned int lastDataBinIndx = _bins.size()-2;
  for(unsigned int ix=1; ix<=lastDataBinIndx; ++ix) {
    counter += _bins[ix].contents;
...
void arHistogram::simpleOutput(ostream & out, bool errorbars, 
			     bool normdata, bool print_dsigma_over_dx) {
  ...
  out << "# " << numPoints << " entries, mean +- sigma = "
      << _globalStats.mean() << " +- " 
      << _globalStats.stdDev()
      << " histogramScale()[fb]= " << CurrentGenerator::current().eventHandler()->histogramScale()/femtobarn << " ";
  if (_havedata) {
  ...
  out << "# xlo xmid xhi ynorm " 
      << (errorbars ? "ynorm_err " : "")
      << (_havedata ? "data " : "")
      << (_havedata && errorbars ? "dataerr " : "")
      << "y_entr";
  if(print_dsigma_over_dx){
    out << " y_entr*histogramScale()/binwidth = dsigma[fb]/dx";
    out << " yerr*histogramScale()/binwidth";
  }
  out << "\n";

  // cerr << "histogramScale()= " << CurrentGenerator::current().eventHandler()->histogramScale()*numPoints/femtobarn << "\n" << flush;

  // the histogram from the event generator
  for(unsigned int ix=1; ix<=lastDataBinIndx; ++ix) {
    double delta = 0.5*(_bins[ix+1].limit-_bins[ix].limit);
    out << _bins[ix].limit << " "
	<< _bins[ix].limit + delta << " "
	<< _bins[ix+1].limit << " " 
	<< yout[ix-1];
    if (errorbars) {
    ...
    if(print_dsigma_over_dx){
      out << " " << _bins[ix].contents/(2*delta)*(CurrentGenerator::current().eventHandler()->histogramScale()/femtobarn);
      out << " " << 0.5*sqrt(_bins[ix].contentsSq)/(2*delta)*(CurrentGenerator::current().eventHandler()->histogramScale()/femtobarn);
    }
    out << '\n';
  }
}

vector<double> arHistogram::dumpBins() const {
...
arHistogram arHistogram::ratioWith(const arHistogram & h2) const {
  const size_t numBins = _bins.size();
  assert( numBins > 2 && numBins == h2._bins.size());
  arHistogram ratio(*this);
...
void arHistogram::normaliseToData()
{
...
void arHistogram::chiSquared(double & chisq, 
...
void arHistogram::normaliseToCrossSection() {
  unsigned int numPoints = _globalStats.numberOfPoints();
  if (numPoints == 0) ++numPoints;
...
void ar2DHistogram::normaliseToData()
{
  double numer(0.),denom(0.);
  unsigned int numPoints = 0;
  for (unsigned int i = 0; i < theNBinY; i++)
    numPoints += histo[i]->_globalStats.numberOfPoints();
  for (unsigned int i = 0; i < theNBinY; i++) {
    for(unsigned int ix=1;ix<histo[i]->_bins.size()-1;++ix)
      {
	double delta = 0.5*(histo[i]->_bins[ix+1].limit-histo[i]->_bins[ix].limit);
	double value = 0.5*histo[i]->_bins[ix].contents / (delta*numPoints);
	if(histo[i]->_bins[ix].dataerror>0.)
	  {
	    double var = sqr(histo[i]->_bins[ix].dataerror);
	    numer += histo[i]->_bins[ix].data * value/var;
	    denom += sqr(value)/var;
	  }
      }
  }
  for (unsigned int i = 0; i < theNBinY; i++)
    histo[i]->_prefactor = numer/denom;
}

void ar2DHistogram::chiSquared(double & chisq, 
			   unsigned int & ndegrees, double minfrac) const {
  chisq =0.;
  ndegrees=0;
  unsigned int numPoints = 0;
  ...
  for (unsigned int i = 0; i < theNBinY; i++) {
    for(unsigned int ix=1;ix<histo[i]->_bins.size()-1;++ix) {
      double delta = 0.5*(histo[i]->_bins[ix+1].limit-histo[i]->_bins[ix].limit);
      double value = 0.5*histo[i]->_prefactor*histo[i]->_bins[ix].contents / (delta*numPoints);
      double error = histo[i]->_bins[ix].dataerror;
      if(error>0.) {
	if(abs(error/histo[i]->_bins[ix].data) < minfrac) error = minfrac*histo[i]->_bins[ix].data;
	double var=sqr(error)
	  + histo[i]->_bins[ix].contentsSq * sqr(0.5*histo[i]->_prefactor / (delta*numPoints));
	chisq += sqr(histo[i]->_bins[ix].data - value) / var;
	++ndegrees;
      }
    }
  }
...
void ar2DHistogram::simpleOutput(ostream & out, bool errorbars, 
			     bool normdata, bool print_dsigma_over_dx) {
  // simple ascii output (eg for gnuplot)
  // work out the y points
  //  unsigned int numPoints = _globalStats.numberOfPoints();  
  unsigned int numPoints = 0;
  for (unsigned int i = 0; i < theNBinY; i++){
    //    cerr << "histogram " << i << " has " << (*histo[i]).visibleEntries() << " visible entries\n" << flush;
    numPoints += (*histo[i]).visibleEntries();
  }
  if (numPoints == 0) ++numPoints;
  double datanorm(1.0); 
  double chisq(0.0), minfrac(0.05);
  ...
  if (normdata) {
    datanorm = 0;
    for (unsigned int i = 0; i < theNBinY; i++){
      if (histo[i]->_havedata) {
	double norm = histo[i]->dataNorm();
	if (norm != -1) datanorm+=norm;
      }
    }
    normaliseToData();
    chiSquared(chisq, ndof, minfrac);    
  ...
  for (unsigned int i = 0; i < theNBinY; i++){
    ...
    double upperlimit = (i+1) * stepy;

    const unsigned int lastDataBinIndx = histo[i]->_bins.size()-2;
    for(unsigned int ix=1; ix<=lastDataBinIndx; ++ix) {
      double delta = 0.5*(histo[i]->_bins[ix+1].limit-histo[i]->_bins[ix].limit);
      delta *= (theUpperY - theLowerY)/theNBinY;
      double value = 0.5*histo[i]->_prefactor*histo[i]->_bins[ix].contents / (delta*numPoints);
      yout.push_back(value);
    }
    ...
    for(unsigned int ix=1; ix<=lastDataBinIndx; ++ix) {
      double delta = 0.5*(histo[i]->_bins[ix+1].limit-histo[i]->_bins[ix].limit);
      out << upperlimit << " "
	  << histo[i]->_bins[ix].limit << " "
	  << histo[i]->_bins[ix].limit + delta << " "
	  << histo[i]->_bins[ix+1].limit << " " 
	  << yout[ix-1];
      if (errorbars) {
	out << " " << 0.5*sqrt(histo[i]->_bins[ix].contentsSq)/(delta*numPoints);
      }
      if (histo[i]->_havedata) {
	out << " " << histo[i]->_bins[ix].data/datanorm;
	if (errorbars)
	  out << " " << histo[i]->_bins[ix].dataerror/datanorm;
      }
      out << " " << histo[i]->_bins[ix].contents;
      if(print_dsigma_over_dx){
	out << " " << histo[i]->_bins[ix].contents/(2*delta)*(CurrentGenerator::current().eventHandler()->histogramScale()/femtobarn);
	out << " " << 0.5*sqrt(histo[i]->_bins[ix].contentsSq)/(2*delta)*(CurrentGenerator::current().eventHandler()->histogramScale()/femtobarn);
      }
      out << '\n';
    }
    out << '\n';

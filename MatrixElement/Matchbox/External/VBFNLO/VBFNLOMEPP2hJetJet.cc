...
#include "Diagrams/DC_Hjj_CC_qBqB.h"
#include "Diagrams/DC_Hjj_CC_qBq.h"
#include "Diagrams/DC_Hjj_CC_qqB.h"
#include "Diagrams/DC_Hjj_CC_qq.h"
#include "Diagrams/DC_Hjj_NC_qBqB.h"
#include "Diagrams/DC_Hjj_NC_qBq.h"
#include "Diagrams/DC_Hjj_NC_qqB.h"
#include "Diagrams/DC_Hjj_NC_qq.h"
#include "Herwig++/MatrixElement/Matchbox/Utility/SU2Helper.h"

using namespace Herwig;


VBFNLOMEPP2hJetJet::VBFNLOMEPP2hJetJet()
  : VBFNLOMEVVJJNeutralBase(), theDecayChannel(0), theNarrowWidth(true) {
}

VBFNLOMEPP2hJetJet::VBFNLOMEPP2hJetJet(int current, bool incoming1, bool incoming2, int decay, bool nw)
  : VBFNLOMEVVJJNeutralBase(current, incoming1, incoming2), theDecayChannel(decay), theNarrowWidth(nw) {
}

VBFNLOMEPP2hJetJet::~VBFNLOMEPP2hJetJet(){
  //  delete theDiagramContainer;
}

int VBFNLOMEPP2hJetJet::Njets() const {return 2;}

IBPtr VBFNLOMEPP2hJetJet::clone() const {
  return new_ptr(*this);
}

IBPtr VBFNLOMEPP2hJetJet::fullclone() const {
  return new_ptr(*this);
}

void VBFNLOMEPP2hJetJet::prepareMomenta(double (& pbar)[14][4],
					double (&) [5],int) const{
  ...
  if (NDecayProducts() == 1) {
    L5MomToDouble( (lastMEMomenta()[4]), &pbar[5][0]);
  }
  else if (NDecayProducts() == 2) {
    L5MomToDouble( (lastMEMomenta()[4]+lastMEMomenta()[5]), &pbar[5][0]);
  }
  else if (NDecayProducts() == 4) {
    L5MomToDouble( (lastMEMomenta()[4]+lastMEMomenta()[5]+lastMEMomenta()[6]+lastMEMomenta()[7]), &pbar[5][0]);
  }
    
  return;
}

void VBFNLOMEPP2hJetJet::VbfnloMe2(double pbar[14][4],int mePartonSign[14],double [5],int,int nlo,double& uucc,double& uuss,double& ddcc,double& ddss,double& udsc,double& ducs) const{
  ...
  if (NDecayProducts() == 1)
    fac *= 16*Constants::pi*BLIPSIVNJ.RMG/(sqr(lastMEMomenta()[4].m2()/GeV2-BLIPSIVNJ.RM2) + sqr(BLIPSIVNJ.RMG));
  ...
  if (NDecayProducts() == 2)
    ...
    L5MomToDouble( (lastMEMomenta()[4]), &v[0][0]);
    L5MomToDouble( (lastMEMomenta()[5]), &v[1][0]);
    L5MomToDouble( (lastMEMomenta()[6]), &v[2][0]);
    L5MomToDouble( (lastMEMomenta()[7]), &v[3][0]);
...
void VBFNLOMEPP2hJetJet::initProcess(const int&) const{
...
double VBFNLOMEPP2hJetJet::colourCorrelatedME2(pair<int,int> ij) const {
...
double VBFNLOMEPP2hJetJet::spinColourCorrelatedME2(pair<int,int>,
...
bool VBFNLOMEPP2hJetJet::noDipole(int emitter, int spectator) const{
...
void VBFNLOMEPP2hJetJet::initDiagramContainers() const {
  if ( theCurrent == neutral ){
    if ( theIncoming1 && theIncoming2 ) theDiagramContainer = RCPtr<DC_Hjj_NC_qq>::Create(DC_Hjj_NC_qq(const_cast<VBFNLOMEPP2hJetJet*>(this)));
    if ( !theIncoming1 && theIncoming2 ) theDiagramContainer = RCPtr<DC_Hjj_NC_qBq>::Create(DC_Hjj_NC_qBq(const_cast<VBFNLOMEPP2hJetJet*>(this)));
    if ( theIncoming1 && !theIncoming2 ) theDiagramContainer = RCPtr<DC_Hjj_NC_qqB>::Create(DC_Hjj_NC_qqB(const_cast<VBFNLOMEPP2hJetJet*>(this)));
    if ( !theIncoming1 && !theIncoming2 ) theDiagramContainer = RCPtr<DC_Hjj_NC_qBqB>::Create(DC_Hjj_NC_qBqB(const_cast<VBFNLOMEPP2hJetJet*>(this)));
  }
  else if ( theCurrent == charged ){
    if ( theIncoming1 && theIncoming2 ) theDiagramContainer = RCPtr<DC_Hjj_CC_qq>::Create(DC_Hjj_CC_qq(const_cast<VBFNLOMEPP2hJetJet*>(this)));
    if ( !theIncoming1 && theIncoming2 ) theDiagramContainer = RCPtr<DC_Hjj_CC_qBq>::Create(DC_Hjj_CC_qBq(const_cast<VBFNLOMEPP2hJetJet*>(this)));
    if ( theIncoming1 && !theIncoming2 ) theDiagramContainer = RCPtr<DC_Hjj_CC_qqB>::Create(DC_Hjj_CC_qqB(const_cast<VBFNLOMEPP2hJetJet*>(this)));
    if ( !theIncoming1 && !theIncoming2 ) theDiagramContainer = RCPtr<DC_Hjj_CC_qBqB>::Create(DC_Hjj_CC_qBqB(const_cast<VBFNLOMEPP2hJetJet*>(this)));
  }
  else
    assert(false);
}

void VBFNLOMEPP2hJetJet::getDiagrams() const {
  if (theInteger == 0){
...
VBFNLOMEPP2hJetJet::diagrams(const DiagramVector &) const {
...
VBFNLOMEPP2hJetJet::colourGeometries(tcDiagPtr diag) const {
  if (theInteger == 0){
    if (!theDiagramContainer){
      initDiagramContainers();
    }
...
void VBFNLOMEPP2hJetJet::doinit(){
  VBFNLOMEVVJJNeutralBase::doinit();

  // ostringstream dname;
  // dname << fullName() << "." << theDiagramContainer->className();
  // if ( ! (generator()->preinitRegister(theDiagramContainer,dname.str()) ) )
  // 		throw InitException() << "DiagramContainer " << dname.str() << " already existing.";
...
void VBFNLOMEPP2hJetJet::doinitrun() {
  VBFNLOMEVVJJNeutralBase::doinitrun();
}

void VBFNLOMEPP2hJetJet::initPSGen() {
...
bool VBFNLOMEPP2hJetJet::generateKinematics(const double * r){
...
int VBFNLOMEPP2hJetJet::NDecayProducts() const {
...
double VBFNLOMEPP2hJetJet::BranchingRatio() const {
  if (theDecayChannel == 1) {
    return BRANCH.BHGAM;
  }
...
int VBFNLOMEPP2hJetJet::nDim() const {
...
ClassDescription<VBFNLOMEPP2hJetJet> VBFNLOMEPP2hJetJet::initVBFNLOMEPP2hJetJet;
// Definition of the static class description member.

void VBFNLOMEPP2hJetJet::persistentOutput(PersistentOStream & os) const {
  os  << theDecayChannel << theNarrowWidth;
}

void VBFNLOMEPP2hJetJet::persistentInput(PersistentIStream & is, int) {
  is  >> theDecayChannel >> theNarrowWidth;
}


void VBFNLOMEPP2hJetJet::Init() {

  static ClassDocumentation<VBFNLOMEPP2hJetJet> documentation
    ("VBFNLOMEPP2hJetJet");

  // static Switch<VBFNLOMEPP2hJetJet,int> interfaceDecayChannel
  //   ("DecayChannel",
  //    "Choose the decay channel that is simulated by VBFNLO.",
  //    &VBFNLOMEPP2hJetJet::theDecayChannel, 0, true, false);
  ...
  // static Switch<VBFNLOMEPP2hJetJet,bool> interfaceNarrowWidth
  //   ("NarrowWidth",
  //    "Choose if VBFNLO simulates the Higgs decay in narrow width approximation",
  //    &VBFNLOMEPP2hJetJet::theNarrowWidth, true, true, false);

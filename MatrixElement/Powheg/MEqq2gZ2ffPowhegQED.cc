...
void MEqq2gZ2ffPowhegQED::Init() {
  ...
  static ClassDocumentation<MEqq2gZ2ffPowhegQED> documentation
    ("The MEqq2gZ2ffPowhegQED class provides a base class for the"
  ...
  static Switch<MEqq2gZ2ffPowhegQED,bool> interfaceFixedAlphaS
    ("FixedAlphaS",
     ...
     &MEqq2gZ2ffPowhegQED::fixedAlphaS_, false, false, false);
...
double MEqq2gZ2ffPowhegQED::collinearQuark(double x, Energy2 mu2, 
				       double jac, double z,
...
double MEqq2gZ2ffPowhegQED::collinearGluon(Energy2 mu2,
				       double jac, double z,
				       double oldPDF, double newPDF) const {
  if(1.-z < 1.e-8) return 0.;
...
vector<double> MEqq2gZ2ffPowhegQED::subtractedReal(pair<double,double> x, double z,
					       double zJac,
...
pair<double,double> 
MEqq2gZ2ffPowhegQED::subtractedMEqqbar(const vector<Lorentz5Momentum> & p,
...
pair<double,double>
MEqq2gZ2ffPowhegQED::subtractedMEgqbar(const vector<Lorentz5Momentum> & p,
				   bool order,bool subtract) const {
  // use the inheriting class to calculate the matrix element
  cPDVector particles(mePartonData());
  if(order) {
...
MEqq2gZ2ffPowhegQED::Singular MEqq2gZ2ffPowhegQED::virtualME() const {
  Singular output;
  output.eps2 = -2;
  output.eps1 = -3;
...
double MEqq2gZ2ffPowhegQED::realME(const cPDVector & particles,

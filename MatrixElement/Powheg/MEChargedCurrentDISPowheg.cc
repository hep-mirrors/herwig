...
// functions of the MEChargedCurrentDISPowheg class.
//

#include "MEChargedCurrentDISPowheg.h"
...
MEChargedCurrentDISPowheg::MEChargedCurrentDISPowheg() 
  : scaleOpt_(1),  muF_(100.*GeV), scaleFact_(1.), contrib_(1), power_(0.1)
{}

int MEChargedCurrentDISPowheg::nDim() const {
  return MEChargedCurrentDIS::nDim()+2;
}

bool MEChargedCurrentDISPowheg::generateKinematics(const double * r) {
  // Born kinematics
  if(!MEChargedCurrentDIS::generateKinematics(r)) return false;
...
IBPtr MEChargedCurrentDISPowheg::clone() const {
  return new_ptr(*this);
}

IBPtr MEChargedCurrentDISPowheg::fullclone() const {
  return new_ptr(*this);
}

void MEChargedCurrentDISPowheg::persistentOutput(PersistentOStream & os) const {
  os << ounit(muF_,GeV) << scaleFact_ << scaleOpt_ << contrib_ << power_;
}

void MEChargedCurrentDISPowheg::persistentInput(PersistentIStream & is, int) {
  is >> iunit(muF_,GeV) >> scaleFact_ >> scaleOpt_ >> contrib_ >> power_;
}

ClassDescription<MEChargedCurrentDISPowheg> 
MEChargedCurrentDISPowheg::initMEChargedCurrentDISPowheg;
// Definition of the static class description member.

void MEChargedCurrentDISPowheg::Init() {

  static ClassDocumentation<MEChargedCurrentDISPowheg> documentation
    ("The MEChargedCurrentDISPowheg class implements the NLO matrix element"
     " for neutral current DIS in the Powheg scheme.");

  static Switch<MEChargedCurrentDISPowheg,unsigned int> interfaceContribution
    ("Contribution",
     "Which contributions to the cross section to include",
     &MEChargedCurrentDISPowheg::contrib_, 1, false, false);
  ...
  static Switch<MEChargedCurrentDISPowheg,unsigned int> interfaceScaleOption
    ("ScaleOption",
     "Option for the choice of factorization (and renormalization) scale",
     &MEChargedCurrentDISPowheg::scaleOpt_, 1, false, false);
  ...
  static Parameter<MEChargedCurrentDISPowheg,Energy> interfaceFactorizationScale
    ("FactorizationScale",
     "Value to use in the event of a fixed factorization scale",
     &MEChargedCurrentDISPowheg::muF_, GeV, 100.0*GeV, 1.0*GeV, 500.0*GeV,
     true, false, Interface::limited);

  static Parameter<MEChargedCurrentDISPowheg,double> interfaceScaleFactor
    ("ScaleFactor",
     "The factor used before Q2 if using a running scale",
     &MEChargedCurrentDISPowheg::scaleFact_, 1.0, 0.0, 10.0,
     false, false, Interface::limited);

  static Parameter<MEChargedCurrentDISPowheg,double> interfaceSamplingPower
    ("SamplingPower",
     "Power for the sampling of xp",
     &MEChargedCurrentDISPowheg::power_, 0.6, 0.0, 1.,
     false, false, Interface::limited);

}

Energy2 MEChargedCurrentDISPowheg::scale() const {
  return scaleOpt_ == 1 ? 
    sqr(scaleFact_)*MEChargedCurrentDIS::scale() : sqr(scaleFact_*muF_);
}

CrossSection MEChargedCurrentDISPowheg::dSigHatDR() const {
  return NLOWeight()*MEChargedCurrentDIS::dSigHatDR();
}

double MEChargedCurrentDISPowheg::NLOWeight() const {

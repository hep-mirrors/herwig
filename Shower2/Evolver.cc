...
void Evolver::showerHardProcess(ShowerTreePtr hard)
{
  ...
  //   hardProcessHardMatrixElementCorrection(particlesToShower,particles);
  ...
  for(cit=hard->outgoingLines().begin();cit!=hard->outgoingLines().end();++cit)
   ...
   if(_splittingGenerator->isInteractionON(ShowerIndex::QCD))
     _partnerFinder->setQCDInitialEvolutionScales(_showerVariables, particles);
   if(_splittingGenerator->isInteractionON(ShowerIndex::QED))
     _partnerFinder->setQEDInitialEvolutionScales(_showerVariables, particles);
   if(_splittingGenerator->isInteractionON(ShowerIndex::EWK))
     _partnerFinder->setEWKInitialEvolutionScales(_showerVariables, particles);
...
//void Evolver::hardProcessHardMatrixElementCorrection(vector<ShowerProgenitor> & particles,
// 						     ShowerParticleVector & hard)
// {
...
//   for(unsigned int ix=0;ix<_mecorrections.size();++ix)
//     {
//       if(_mecorrections[ix]->canHandle(particles))
// 	{
// 	  if(_currentme)
// 	    {
...
// 		{output << particles[ix].particle->PDGName() << " ";}
// 	      output << " -> ";
// 	      for(unsigned int ix=2;ix<particles.size();++ix)
// 		{output << particles[ix].particle->PDGName() << " ";}
// 	      output << "in Evolver::hardProcessHardMatrix"
// 		     << "ElementCorrection() using the first one\n";
// 	      Exception() << output << Exception::runerror;
// 	    }
...
//     _currentme->applyHardMatrixElementCorrection(particles);
//   // add to initial particles if needed
//   if(particles.size()-hard.size()>1) 
//     throw Exception() << "More than one particle added in Evolver"
// 		      << "::hardProcessHardMatrixElementCorrection()"
// 		      << Exception::runerror;
//   else if(hard.size()<particles.size()) 
//     hard.push_back(particles.back().particle);
// }
			     ...
			     const bool specialDecay)
{
  ...
  while (vetoed) 
    {
      vetoed = false; 
      fb=_splittingGenerator->chooseForwardBranching(*particle, specialDecay);
  ...
  //
  timeLikeShower(showerProduct1,specialDecay);
  // shower the second particle
  //
  //   need to set rho here
  //
  timeLikeShower(showerProduct2,specialDecay);
...
bool Evolver::spaceLikeShower(tShowerParticlePtr particle)
{
  Timer<1006> timer("Evolver::spaceLikeShower");
   ...
   // now reconstruct this particle and the children
   bb.kinematics->updateParent(newParent,newParent->children());
  ...
  for(cit = hard->incomingLines().begin();cit!=hard->incomingLines().end();++cit)
    {
      ...
      if(!particle->isFinalState()&&particle->isFromHardSubprocess()&&
	 isISRadiationON())
	{

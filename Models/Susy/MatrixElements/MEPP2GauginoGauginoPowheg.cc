...
//   cerr << "mandelstam" << sHat()/GeV2<< " " <<th/GeV2<< " " <<uh/GeV2 << "\n";
...
//        << test/loWeight() << "\n";
  ...
  // the rest of it
  double B0tg=SusyLoopIntegral::B0 (th,mg,ZERO,scale2);
  ...
  double B0ug=SusyLoopIntegral::B0 (uh,mg,ZERO,scale2);
  ...
  values[4] +=  4.*SusyLoopIntegral::B0(th,ms,ZERO,scale2)*
    (-b_t*(1.+m12/t1-(th+ms2)/ts+m22/t2)
     -b_tm/sHat()*ts+b_tx*th*ts/t1/t2);
  // cerr << "testing E " << values[4] << "\n";
  values[5] +=  4.*SusyLoopIntegral::B0(uh,ms,ZERO,scale2)*
    (-b_u*(1.+m12/u1-(uh+ms2)/us+m22/u2)
     -b_um/sHat()*us+b_ux*uh*us/u1/u2);
  ...
  values[7] +=  4.*B012s/kaellen*b_tm*ts/sHat()   *(sHat()*th-m12*u2-m22*t1);
  values[7] +=  4.*B012s/kaellen*b_tx*m12*ts/t1/t2*(sHat()*th-m12*t2-m22*u1);
  values[7] += -4.*B012s/kaellen*b_um*us/sHat()   *(sHat()*th-m12*u2-m22*t1);
  values[7] +=  4.*B012s/kaellen*b_ux*m12*us/u1/u2*(sHat()*uh-m12*u2-m22*t1);
  values[7] +=  4.*B012s/kaellen*b_um2*us/sHat()   *(sHat()*th-m12*u2-m22*t1);
  values[7] += -4.*B012s/kaellen*b_ux2*us*m12/u1/u2*(sHat()*uh-m12*u2-m22*t1);
  values[7] += -4.*B012s/kaellen*b_tm4/sHat()*ts*   (sHat()*th-m12*u2-m22*t1);
  values[7] += -4.*B012s/kaellen*b_tx4*ts*m12/t1/t2*(sHat()*th-m12*t2-m22*u1);
  ...
  values[8] += -4.*B022s/kaellen*b_tm*ts/sHat()*(uh*sHat()-m12*u2-m22*t1);
  values[8] +=  4.*B022s/kaellen*b_tx*m22*ts/t1/t2*(sHat()*th-m12*u2-m22*t1);
  values[8] +=  4.*B022s/kaellen*b_um*us/sHat()   *(sHat()*uh-m12*u2-m22*t1);
  values[8] +=  4.*B022s/kaellen*b_ux*us*m22/u1/u2*(sHat()*uh-m12*t2-m22*u1);
  values[8] += -4.*B022s/kaellen*b_um2*us/sHat()*   (uh*sHat()-m12*u2-m22*t1);
  values[8] += -4.*B022s/kaellen*b_ux2*m22*us/u1/u2*(sHat()*uh-m12*t2-m22*u1);
  values[8] += +4.*B022s/kaellen*b_tm4*ts/sHat()*   (sHat()*uh-m12*u2-m22*t1);
  values[8] += -4.*B022s/kaellen*b_tx4*m22*ts/t1/t2*(sHat()*th-m12*u2-m22*t1);
  ...
  values[9] +=  4.*B0sss/kaellen*b_um2*(uh-th)*us;
  values[9] +=  4.*B0sss/kaellen*b_ux2*us/u1/u2*(2.*m12*m22*sHat()-sqr(m12-m22)*uh+(m12+m22)*sHat()*uh);
  values[9] +=  4.*B0sss/kaellen*b_tm4*ts*(th-uh);
  values[9] +=  4.*B0sss/kaellen*b_tx4*ts/t1/t2*(2.*m12*m22*sHat()-sqr(m12-m22)*th+(m12+m22)*sHat()*th);
  values[9] +=  2.*B0sss*b_s*(1.+2.*(mg2-ms2)/sHat());
  values[9] +=  4.*B0sss*(b_ux2*uh*us/u1/u2+b_tx4*th*ts/t1/t2);
  ...
  values[11] +=  4.*B0s/kaellen*b_tm*ts*(uh-th);
  values[11] += -4.*B0s/kaellen*b_tx*ts/t1/t2*(2.*m12*m22*sHat()-sqr(m12-m22)*th+(m12+m22)*sHat()*th);
  values[11] +=  4.*B0s/kaellen*b_um*us*(th-uh);
  values[11] += -4.*B0s/kaellen*b_ux*us/u1/u2*(2.*m12*m22*sHat()-sqr(m12-m22)*uh+(m12+m22)*sHat()*uh);
  values[11] += -6.*B0s*b_s;
  values[11] += -4.*B0s*(b_tx*th*ts/t1/t2+b_ux*uh*us/u1/u2);
  // cerr << "testing L " << values[11] << "\n";
  double C01tsg = ( sHat()*SusyLoopIntegral::C0(m12,eps,th,eps2,ms  ,mg  )).real();
  values[12] +=  4.*C01tsg*Cv[0]*b_t/sHat()/t1*m1*mg*(mg2-ms2-t1);
  values[12] +=  2.*C01tsg*b_um2*us/sqr(sHat())*(2.*(mg2-ms2)-t1);
  values[12] +=  2.*C01tsg*b_ux2*us/sHat()/u1/u2/t1*(-(mg2-ms2)*(t1*(th+m12)+2*sHat()*m12)+(ms2-sHat())*t1*t1);
  // cerr << "testing M " << values[12] << "\n";
  double C02tsg = ( sHat()*SusyLoopIntegral::C0(m22,eps,th,eps2,ms  ,mg  ) ).real();
  values[13] +=  4.*C02tsg*Cv[1]*b_t*m2*mg/sHat()/t2*(mg2-ms2-t2);
  values[13] +=  2.*C02tsg*b_um2*us/sqr(sHat())*(2.*(mg2-ms2)-t2);
  values[13] +=  2.*C02tsg*b_ux2*us/sHat()/u1/u2/t2*(-(mg2-ms2)*(t2*(th+m22)+2*sHat()*m22)+(ms2-sHat())*t2*t2);
  // cerr << "testing N " << values[13] << "\n";
  double C01usg = ( sHat()*SusyLoopIntegral::C0(m12,eps,uh,eps2,ms  ,mg  ) ).real();
  values[14] +=  4.*C01usg*Cv[2]*b_u*m1*mg/sHat()/u1*(mg2-ms2-u1);
  values[14] +=  2.*C01usg*b_tm4 *ts/sqr(sHat())*(2.*(mg2-ms2)-u1);
  values[14] +=  2.*C01usg*b_tx4*ts/t1/t2/sHat()/u1*(-(mg2-ms2)*((uh+m12)*u1+2*sHat()*m12)+(ms2-sHat())*u1*u1);
  // cerr << "testing O " << values[14] << "\n";
  double C02usg = ( sHat()*SusyLoopIntegral::C0(m22,eps,uh,eps2,ms  ,mg  ) ).real();
  values[15] +=  4.*C02usg*Cv[3]*b_u*m2*mg/sHat()/u2*(mg2-ms2-u2);
  values[15] +=  2.*C02usg*b_tm4*ts/sqr(sHat())*(2.*(mg2-ms2)-u2);
  values[15] +=  2.*C02usg*b_tx4*ts/t1/t2/sHat()/u2*(-(mg2-ms2)*((uh+m22)*u2+2*sHat()*m22)+(ms2-sHat())*u2*u2);
  // cerr << "testing P " << values[15] << "\n";
  double C0Dt1 = SusyLoopIntegral::C0div(th,m1,m2,ms,scale2);
  double C0Dt2 = SusyLoopIntegral::C0div(th,m2,m1,ms,scale2);
  values[16] +=  4.*b_t*(C0Dt1/t1*(ms2-m12)+C0Dt2/t2*(ms2-m22));
  values[16] +=  4.*b_tm*ts*(C0Dt1/t1+C0Dt2/t2 );
  values[16] +=  2.*b_tx*ts/t1/t2*(C0Dt1*(th-ms2+m12-m22)+C0Dt2*(th-ms2-m12+m22));
  // cerr << "testing Q " << values[16] << "\n";
  double C0Du1 = SusyLoopIntegral::C0div(uh,m1,m2,ms,scale2);
  double C0Du2 = SusyLoopIntegral::C0div(uh,m2,m1,ms,scale2);
  values[17] += -4.*b_u*(C0Du1/u1*(m12-ms2)+C0Du2/u2*(m22-ms2));
  values[17] +=  4.*b_um*us*(C0Du1/u1+C0Du2/u2);
  values[17] +=  2.*b_ux*us/u1/u2*(C0Du1*(uh-ms2+m12-m22)+C0Du2*(uh-ms2-m12+m22));
  ...
  values[19] +=  2.*C0div*b_tx*ts/t1/t2*(m12+m22-ms2-th);
  values[19] +=  2.*C0div*b_ux*us/u1/u2*(m12+m22-ms2-uh);
  // cerr << "testing T " << values[19] << "\n";
  double C0120s0=( sHat()*SusyLoopIntegral::C0(m12,m22,sHat(),eps2,ms  ,eps2) ).real();
  Energy2 s= sHat(), u=uh, t=th;


  values[20] += +4.*C0120s0/kaellen*b_tm*ts/sHat()*(2*m12*m22+ms2*(uh-th)-sqr(uh)-uh*th);
  ...
  values[20] += +C0120s0/kaellen*b_tx*ts/t1/t2*(ms2*(t2*m22+sHat()*(m12-m22)-4*sHat()*t2)
					 +3*sHat()*(-u1*m22-sHat()*th)-uh*m22*t2);
  ...
  values[20] +=     C0120s0*b_ux*us/sHat()/u1/u2*(2.*th*(m12+m22)-4.*m12*m22+2.*ms2*(uh-th)
					   +sHat()*(2.*uh-m12+m22));
  ...
  values[21] +=  2.*C012s0s*b_um2*us/sqr(sHat())*(2.*(ms2-mg2)+th-uh);
  ...
  values[21] +=  2.*C012s0s*b_tm4*ts/sqr(sHat())*(2.*(ms2-mg2)+uh-th);
  values[21] +=  2.*C012s0s*b_tx4*ts/sHat()/t1/t2*((mg2-ms2)*(uh+th)+2*m12*sHat()+(sHat()-ms2)*(uh-th));
  // cerr << "testing V " << values[21] << "\n";
  double D0ts = SusyLoopIntegral::D_div(th,m1,m2,ms,sHat(),scale2);
  ...
  double D0us = SusyLoopIntegral::D_div(uh,m2,m1,ms,sHat(),scale2);
  ...
  double D0st = sHat()*tg*SusyLoopIntegral::D_fin(eps,eps,m12,m22,sHat(),th,ms,mg,ms,eps2);
  ...
  double D0su = sHat()*ug*SusyLoopIntegral::D_fin(eps,eps,m22,m12,sHat(),uh,ms,mg,ms,eps2);

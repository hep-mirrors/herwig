// -*- C++ -*-
//
// FourJetCorrelations.cc is a part of Herwig++ - A multi-purpose Monte Carlo event generator
...
// functions of the FourJetCorrelations class.
//
...
void FourJetCorrelations::analyze(const tPVector &) {
  ...
  jetFinder()->use(lastEvent());
  jetFinder()->findJetsY();

  if (jetFinder()->getNJets() != 4) return;

  jetFinder()->sortEnergy();

  vector<Lorentz5Momentum> 
    vJets (jetFinder()->jets().begin(),jetFinder()->jets().end());

  book(cosAlpha34(vJets),"CosAlpha34");
  book(abs(cosChiBZ(vJets)),"CosChiBZ");
  book(cosPhiKSW(vJets),"CosPhiKSW");
  book(abs(cosThetaNR(vJets)),"CosThetaNR");

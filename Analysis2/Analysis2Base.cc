// -*- C++ -*-
//
// Analysis2Base.cc is a part of Herwig++ - A multi-purpose Monte Carlo event generator
...
// functions of the Analysis2Base class.
//
...
void Analysis2Base::insert (const string& name,
  ...
  if (!fromdata) {
    ...
    if (!log)
      insertObservable(name,new_ptr(Histogram2(binlower,binhigher,numbins,"Herwig++")),options);
    else {
      if (binlower == 0) {
	Repository::clog() << error;
	return;
...
void Analysis2Base::finish (const string& name,
  ...
  if (!_bookPerSubprocess) {
    ...
    if (theHisto->haveChannel("Herwig++") && normMode != NoNormalisation) {

      if (normMode == NormaliseToData && !theHisto->haveChannel(data))
	normMode = NormaliseToXSec;
      switch (normMode) {
      case NormaliseToUnity:
	theHisto->normalise("Herwig++");
	break;
      case NormaliseToXSec:
	theHisto->normaliseToCrossSection("Herwig++");
	break;
      case NormaliseToData:
	theHisto->normalise("Herwig++",data);
	break;
      }
    ...
    for (unsigned int i = minMult(); i<= maxMult(); ++i) {
      ostringstream chName; chName << "Herwig++-" << i;    
      sum += theHisto->channel(chName.str());
    }
    
    theHisto->removeChannel("Herwig++");
    theHisto->insertChannel("Herwig++", sum);
    ...
    if (normMode != NoNormalisation) {
      ...
      fromIntegral = theHisto->integrate("Herwig++");
      if (normMode == NormaliseToData && !theHisto->haveChannel(data))
	normMode = NormaliseToXSec;
      switch (normMode) {
      case NormaliseToUnity:
	...
	break;
      ...
      theHisto->channel("Herwig++") /= fromIntegral;
      theHisto->channel("Herwig++") *= toIntegral;
  ...
  if (theHisto->haveChannel("Herwig++") && theHisto->haveChannel(data)) {

    // ratio
    theHisto->insertChannel("delta (Herwig++,"+data+")",theHisto->channel("Herwig++").delta(theHisto->channel(data)));
    
    // chi2
    theHisto->insertChannel("chi2 (Herwig++,"+data+")",theHisto->channel("Herwig++").chi2(theHisto->channel(data)));
    if (!combined)
      generator()->log() << name << " : chi2/DOF (Herwig++,"+data+") = "
			 << theHisto->channel("chi2 (Herwig++,"+data+")").average(theHisto->binning()).first
			 << endl;
    else
      Repository::clog() << name << " : chi2/DOF (Herwig++,"+data+") = "
			 << theHisto->channel("chi2 (Herwig++,"+data+")").average(theHisto->binning()).first
			 << endl;
  }
...
void Analysis2Base::combineObservable (const string& prefix,
  ...
  sum->combine(prefix,name,numRuns,dataChannel,"Herwig++");
